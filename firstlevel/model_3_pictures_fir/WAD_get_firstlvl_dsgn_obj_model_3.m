%% WAD_get_firstlvl_dsgn_obj_model_3
%
% This script contains a function that defines a number of fields of 
% the CANlab style first level DSGN structure array for the WAD study on 
% neck-related pictures of movements (taken from the PFActS-C) that are 
% presented and vary in the degree to which they are perceived as fearful.
% This function is used in WAD_first_m1_s1_spm_fit_firstlvl_models.m to
...
% to be used by WAD_first_m1_s1_spm_fit_firstlvl_models.m
%
...
%
%__________________________________________________________________________
% @(#)% WAD_get_firstlvl_dsgn_obj_model_3.m         v1.0       
% last modified: 2021/04/08
%
%
%% function code
function DSGN = WAD_get_firstlvl_dsgn_obj_model_3()
    % INPUT
    % required fields
    DSGN.metadata = "WAD first level analysis model 3, i.e. modeling 3 conditions for high, moderate, neutral fear as 6 second events (= duration of picture + imagine), with fir basis set"; % field for annotation with study info, or whatever you like
    DSGN.modeldir = 'C:\Users\lukas\Dropbox (Dartmouth College)\4.Iris_WAD_MRI\firstlevel\model_3_pictures_fir'; % directory where you want to write first level results
    ...
    c=0;
    c=c+1;DSGN.conditions{c}={'high_fear' 'moderate_fear' 'neutral_fear'};
    c=c+1;DSGN.conditions{c}={'high_fear' 'moderate_fear' 'neutral_fear'};
    % optional fields
%     DSGN.pmods = {{}}; % cell array (one cell per session) of cell arrays (one cell per condition) of cell arrays (one cell per modulator) of MAT-file names
    DSGN.convolution.type = 'fir';
    DSGN.convolution.windowlength = 10; % minimum interval between onsets of two trials - TO BE CHECKED WITH TOR/MARTIN/BOGDAN
    DSGN.convolution.order = 10; % TR = 1 hence identical to windowlength? - TO BE CHECKED WITH TOR/MARTIN/BOGDAN
    DSGN.convolution.keepduration = false; % default: false, which is desirable in case of fir I guess
...
%     DSGN.singletrialsall = false; % default: false; if true, set DSGN.singletrials to true for all conditions
    DSGN.modelingfilesdir = 'model_3_pictures_fir'; % name of subfolder which will be created within directory containing functional files where .mat files containing fields of DSGN structure will be saved
